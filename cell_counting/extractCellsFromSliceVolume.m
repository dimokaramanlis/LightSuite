function cell_locations = extractCellsFromSliceVolume(datavolume, opts)
...
for islice = 1:NbatchesZ
    %----------------------------------------------------------------------    
    istartz    = (islice - 1)*batchsizez + 1;
    iendz       = min(istartz + batchsizez - 1, Nslices);
    startbuffz = (islice > 1) * buffsizez;
    endbuffz   = (islice < NbatchesZ) * buffsizez;
    if endbuffz > 0
        endbuffz = min(endbuffz, Nslices - islice*batchsizez);
    end
    ...
    for ibatchy = 1:NbatchesY
        ...
        for ibatchx = 1:NbatchesX
            ...
            %----------------------------------------------------------------------
            [ccents, pdiscard] = cellDetector2D(datgpu, cellradius, sigmause, thresuse);
            if ~isempty(ccents)
