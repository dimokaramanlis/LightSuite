...
if nargin < 2
    ...
    coluse = varargin{1};
    idchan = find(contains(sliceinfo.channames, coluse));
    idchan = find(idchan == chansmap);
end

errormessage = sprintf('%s ', sliceinfo.channames{chansmap});
errormessage = sprintf('Select a single channel! Available channels are: %s', errormessage);
assert(~isempty(idchan), errormessage)

fprintf('Channel %s selected to plot for deepslice... ', sliceinfo.channames{chansmap(idchan)})
